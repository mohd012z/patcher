...
local un_ads="${util_dir}/AddIdComments";
[ -f "$un_ads" ] || ab_ort "\"AddIdComments\" binary is not installed!
   Please install it before using this patch.";
[ -f "${un_ads}.jar" ] || ab_ort "\"AddIdComments\" binary is not installed!
   Please install it before using this patch.";
setspan_green "
  AddIdComments working...
";
local F="$(pwd)";
