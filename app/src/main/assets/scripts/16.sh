...
local un_ads="${util_dir}/SignatureKiller";
[ -f "$un_ads" ] || ab_ort "\"SignatureKiller\" binary is not installed!
   Please install it before using this patch.";
[ -f "${un_ads}.jar" ] || ab_ort "\"SignatureKiller\" binary is not installed!
   Please install it before using this patch.";
setspan_green "
  SignatureKiller working...
";
local F="$(pwd)";
